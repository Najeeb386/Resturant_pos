    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Cash Received & Change Return Option
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('Cash Received & Return Change', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.teal)),
                    if (provider.cashReceived > 0)
                      InkWell(
                        onTap: () => provider.setCashReceived(0.0),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Enter cash received (e.g. ${provider.total.toStringAsFixed(0)})',
                  ...
                  onChanged: (val) {
                    provider.setCashReceived(double.tryParse(val) ?? 0.0);
                  },
                ),
                if (provider.cashReceived > 0) ...[
                  const SizedBox(height: 6),
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: provider.cashReceived >= provider.total ? Colors.teal.shade700 : Colors.amber.shade800,
                      borderRadius: BorderRadius.circular(6),
                    ),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          provider.cashReceived >= provider.total ? 'Return Change:' : 'Remaining Due:',
                          style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11),
                        ),
                        Text(
                          provider.cashReceived >= provider.total
                              ? 'Rs ${(provider.cashReceived - provider.total).toStringAsFixed(2)}'
                              : 'Rs ${(provider.total - provider.cashReceived).toStringAsFixed(2)}',
